...
const authService = {
    ...
    oauthLogin: async (token: string) => {
        const res = await api.post(ENDPOINTS.AUTH.OAUTH_LOGIN, { token });
        return res.data.data;
    },

    oauthRegisterTrainee: async (data: { token: string; phone?: string; full_name?: string }) => {
        const res = await api.post(ENDPOINTS.AUTH.OAUTH_REGISTER_TRAINEE, data);
        return res.data.data;
    },

    oauthRegisterTrainer: async (data: RegisterTrainerPayload & { token: string }) => {
        const res = await api.post(ENDPOINTS.AUTH.OAUTH_REGISTER_TRAINER, data);
        return res.data; // this endpoints returns data with a message property, so we might want res.data.data or res.data depending on ApiResponse wrapper
        // wait, backend uses ApiResponse.created(res, result, result.message) which returns { success, data, message }
        // So res.data.data contains { message: string }. 
        // Let's just return res.data to keep access to message. 
    },
